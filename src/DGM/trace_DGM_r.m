    ...
    switch floor(element_label(ie)/1000)
        case {0,8}
            ...
            for i_thetaphi=1:nb_theta
                ...
                Phi_elem(:,1)=Phi_elem(:,1)+Phi_e*exp(-j*k_air*(n_phi.'*(coord_elem(:,1)-x_centre)))*q(1+ondes_element(ie)*(i_thetaphi-1));
                Phi_elem(:,2)=Phi_elem(:,2)+Phi_e*exp(-j*k_air*(n_phi.'*(coord_elem(:,2)-x_centre)))*q(1+ondes_element(ie)*(i_thetaphi-1));
                Phi_elem(:,3)=Phi_elem(:,3)+Phi_e*exp(-j*k_air*(n_phi.'*(coord_elem(:,3)-x_centre)))*q(1+ondes_element(ie)*(i_thetaphi-1));
            end
