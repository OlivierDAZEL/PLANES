% radiative_boundary_application.m
%
...
for ie=1:nb.radiative
    ...
    a=nodes(edges.radiative(ie,1),1:2)';
    b=nodes(edges.radiative(ie,2),1:2)';
    
    %%%%% Element linked to the edge
    
    num_element=edges.radiative(ie,3)
    center_element=mean(nodes(nonzeros(elem.nodes(num_element,:)),1:2))'
    
    index_element  =((1:data_model.theta_DGM.nb)-1)+dof_start_element(num_element)
    ...
    for i_champs=1:data_model.theta_DGM.nb
        ...
        if (cos(vec_theta(i_champs))*nx+sin(vec_theta(i_champs))*ny)>0
            i_champs
            cos(vec_theta(i_champs))
            sin(vec_theta(i_champs))
            A(index_element(i_champs),:)=0;
            A(:,index_element(i_champs))=0;
            A(index_element(i_champs),index_element(i_champs))=1;
        end
    end
