...
% along with this program.  If not, see <http://www.gnu.org/licenses/>.% Dans le poreux
...
M_an(1,1)=sin(k_air*x);
M_an(1,2)=-sin(delta_eq*x);
M_an(1,3)=-cos(delta_eq*x);
% Continuite de la pression
M_an(2,1)=air.K*k_air*cos(k_air*x);
M_an(2,2)=-K_eq_til*delta_eq*cos(delta_eq*x);
M_an(2,3)= K_eq_til*delta_eq*sin(delta_eq*x);
...
M_an(3,2)= sin(delta_eq*x);
M_an(3,3)= cos(delta_eq*x);
F_an(3,1)=-1;
...
sigma_p=air.K*k_air*A*cos(k_air*x_EF);
