...
for ie=1:nb_loads
    ...
    switch typ
        case {10}
            ...
            Omega_F.uy=1j*k_z/(rho_0*omega^2);
            ...
            for i_R=1:nb_R
                ...
                index_F_elem=find(index_force);
                index_F_global=index_force(index_F_elem);
                Omega.uy=1j*vec_k_z(i_R)/(rho_0*omega^2);
                A(index_F_global,nb_dof_FEM+i_R)=A(index_F_global,nb_dof_FEM+i_R)+Omega.uy*F3(index_F_elem);
        ...
        case {11}
            ...
            for i_R=1:nb_R
                ...
                index_F_global=index_force(index_F_elem);
                Omega.p=-1;
                A(index_F_global,nb_dof_FEM+i_R)=A(index_F_global,nb_dof_FEM+i_R)+Omega.p*F3(index_F_elem);
                A(nb_dof_FEM+i_R,index_F_global)=A(nb_dof_FEM+i_R,index_F_global)+        F3(index_F_elem)';
            end
        case {13}
            DtN_plate_R=1;
        ...
        case {21}
            ...
            for i_T=1:nb_T
                ...
                index_F_global=index_force(index_F_elem);
                Omega.p=1;
                A(index_F_global,nb_dof_FEM+nb_R+i_T)=A(index_F_global,nb_dof_FEM+nb_R+i_T)+Omega.p*F3(index_F_elem);
                A(nb_dof_FEM+nb_R+i_T,index_F_global)=A(nb_dof_FEM+nb_R+i_T,index_F_global)+        F3(index_F_elem)';
            end
        otherwise
            disp('Unknown load')
            stop
...
if DtN_elas_R==1
    for i_R=1:nb_R
        A(i_R+nb_dof_FEM,i_R+nb_dof_FEM)=-period*(1i*vec_k_z(i_R))/(rho_0*omega^2);
    end
    F(nb_dof_FEM+1)=F(nb_dof_FEM+1)-period*(1i*k_z)/(rho_0*omega^2);
end
...
if DtN_plate_R==1
    ...
    Omega_moins=[0;-j*k_z/(rho_0*omega^2);-1 ;0 ];
    Omega_plus=transfer_force(k_x,omega,Omega_moins);
    ...
    for i_R=1:nb_R
        ...
        Omega_moins=[0 0;j*vec_k_z(i_R)/(rho_0*omega^2) 0;-1 0;0 1];
        [Omega_plus,T_back_i]=transfer_unknowns(vec_k_x(i_R),omega,Omega_moins);
...
if DtN_elas_T==1
    for i_T=1:nb_T
        A(nb_R+i_T+nb_dof_FEM,nb_R+i_T+nb_dof_FEM)=period*(1i*vec_k_z_t(i_T))/(rho_0*omega^2);
    end
end
