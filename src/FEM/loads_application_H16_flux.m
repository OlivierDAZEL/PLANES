...
for ie=1:nb.loads
    ...
    indice_champs_y=index_p([3 4 8 7]);
    ...
    switch typ
        case {3}
            if sort(loads(ie,1:2))==sort(elements(loads(ie,3),1:2)) % Interface node 1 node 2: bottom edge
                ...
                line_PP=2;
                PP=-PP(line_PP,:) %\d p/\p n =-v_y
                FF=-FF(line_PP)
                for i_test=1:4
                    eval(['Psi_test=Psi_',num2str(i_test),'_x'])
                    F(indice_test(i_test))=F(indice_test(i_test))+integrate_polynom(Psi_test,lx)*FF;
                    for i_champs=1:4
                        ...
                        A(indice_test(i_test),indice_champs_0(i_champs))=A(indice_test(i_test),indice_champs_0(i_champs))-integrate_polynom(multiply_polynom(Psi_test,temp),lx);
                        eval(['temp=PP(2)*(-1/(j*air.rho*omega))*Psi_',num2str(i_champs),'_x'])
                        A(indice_test(i_test),indice_champs_y(i_champs))=A(indice_test(i_test),indice_champs_y(i_champs))-integrate_polynom(multiply_polynom(Psi_test,temp),lx);
                    end
                end
            else
