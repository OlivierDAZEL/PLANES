...
M_an(1,1)=K_eq_til*delta_1*mu_1*cos(delta_1*x);
M_an(1,2)=K_eq_til*delta_2*mu_2*cos(delta_2*x);
M_an(1,3)=1;
F_an(1,1)=-1;
...
% Nullit? in vacuo
M_an(3,1)=mu_1*sin(delta_1*x);
M_an(3,2)=mu_2*sin(delta_2*x);
M_an(3,3)=-(j*k_0/(omega^2*rho_0));
F_an(3,1)=M_an(3,3);
